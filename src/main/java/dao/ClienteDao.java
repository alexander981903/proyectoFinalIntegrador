...
public class ClienteDao {
    ...
    public boolean insertarCliente(Cliente cliente) {
        String sql = "INSERT INTO cliente (nombre, apellido, email, telefono, dni) VALUES (?, ?, ?, ?,?)";
    ...
    public boolean actualizarCliente(Cliente cliente) {
        String sql = "UPDATE cliente SET nombre = ?, apellido = ?, email = ?, telefono = ?, dni = ? WHERE idCliente = ?";
        try (PreparedStatement ps = conexion.prepareStatement(sql)) {
            ps.setString(1, cliente.getNombre());
            ps.setString(2, cliente.getApellido());
            ps.setString(3, cliente.getEmail());
     ...
     * 
     * @param dato El nombre o dni que se desea buscar en la base de datos.
     * @return Una lista de objetos Cliente que coinciden con el nombre buscado.
     */
    public ArrayList<Cliente> buscarClientesPorNombre(String dato) {
        ArrayList<Cliente> clientes = new ArrayList<>();
        String sql = "SELECT * FROM cliente WHERE nombre LIKE ? OR dni LIKE ?";
